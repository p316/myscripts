...
#			if [ $app_name -eq 'data-activity-loader' ||  $app_name -eq 'data-event-sync' \	
#				$app_name -eq 'data-search-loader' || $app_name -eq 'notification-engine'] ; then
 #                                 cmd=$(curl -vk --connect-timeout 5 https://$host_name:8080 &>/dev/null)
#                        fi
       		fi
        res=$?
        if [ $header -eq 1 ] ; then
